...
begin
  ...
  begin -- begin of process ( )
    ...
    -- PIPT: cpu.vhd relocates the address upstream of the cache, so a.a is
    -- already a PA; index and tag are both physical in every mode.
    ...
    -- ------------------------------------------------------------------------
    -- Snoop compares a PA: both the broadcast address (saout_al1, taken from
    -- a.a) and the stored tag (from ma0, likewise a.a) come from the same
    -- upstream-relocated physical address, so they match under AT=1 too.
...
this_c <= this;
  ...
  ry_ccl.ten0 <= '1';
  ry_ccl.tag0 <= (CACHE_PA_TAG_WIDTH-CACHE_TAG_WIDTH-1 downto 0 => '0')
