...
func EmitDelta(insns []ir.Insn) string {
	...
	for _, in := range insns {
		...
		for _, o := range in.Operands {
			args = append(args, argCode(o))
		}
		args = append(args, "0") // sh_table arg arrays are zero-terminated
		fmt.Fprintf(&b, "{%q,{%s},{%s},%s},\n",
			in.Mnemonic, strings.Join(args, ","), nibbles(in), in.Arch.GASMask())
	}
	return b.String()
}

func argCode(o operand.Operand) string {
	switch o.Class {
	case operand.GPR:
		if o.Letter == 'n' {
			return "A_REG_N"
	...
	case operand.MemR0:
		if o.Letter == 'n' {
			return "A_IND_R0_REG_N"
		}
		return "A_IND_R0_REG_M"
...
func nibbles(in ir.Insn) string {
	w := in.Words[0]
	out := make([]string, 4)
	for n := 0; n < 4; n++ {
		hi := n * 4
