...
entity dcache_color_tb is end entity;
...
architecture tb of dcache_color_tb is
  ...
  signal mem_lock, mem_ddrburst, mem_ack_r : std_logic;
...
begin
  ...
  dut : dcache_cacheable_mux
    port map (clk125 => clk, clk200 => clk, rst => rst, ctrl => ctrl,
              cpu_o => cpu_o, a_mmu => a_mmu, lock => '0', cpu_i => cpu_i,
              mem_o => mem_o, mem_lock => mem_lock, mem_ddrburst => mem_ddrburst,
              mem_i => mem_i, mem_ack_r => mem_ack_r);

  -- behavioral 1-wait memory (same shape as dcache_check_tb)
  mem_o_d5 <= mem_o after 5 ns;
  process(mem_o, mem_o_d5) begin
    if mem_o_d5.en = '1' and mem_o.en = '1' and mem_o_d5.a = mem_o.a then
      ddr_rdy <= '1'; else ddr_rdy <= '0'; end if;
  end process;
  mem_i.ack <= ddr_rdy; mem_ack_r <= ddr_rdy;
  process(mem_o, ddr_rdy) begin
    if ddr_rdy = '1' then mem_i.d <= ddr_mem(widx(mem_o.a));
    else mem_i.d <= (others => '0'); end if;
  end process;
  process(clk) begin
