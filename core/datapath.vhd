...
entity datapath is
 ...
 port (
       ...
       -- P4 PRIVILEGE VIOLATION export (J4+PRIV_ARCH; constant '0'
       -- otherwise, so J1/J2 see a tied-off port exactly like
       -- tlb_squash_o). p4_viol_o pulses for one cycle, ONE CYCLE AFTER a
       -- user-mode (SR.MD='0') data access to P4 was launched and refused
       -- by the MMU-register block below; p4_viol_wr_o carries its
       -- direction ('1' = store). cpu.vhd turns the pulse into a
       -- P4_USER_R/P4_USER_W exception.
       --
       -- REGISTERED, not combinational, and that is load-bearing twice
       -- over. (1) cpu.vhd feeds tlb_exc_pend straight back INTO this
       -- process, so a combinational export would close a combinational
       -- loop through cpu.vhd's exception process. (2) A D-side TLB fault
       -- is likewise detected one cycle after its launch (cpu.vhd sees
       -- sig_db_o, the REGISTERED bus request), so a one-cycle delay puts
       -- this fault at exactly the pipeline point squash_arm and the
       -- ex_if_pc restart-PC derivation already expect.
       --
       -- There is deliberately NO faulting-VA port beside these: the
       -- refusal writes the VA to TEA in the same evaluation that raises
       -- p4_viol_c, and cpu.vhd reads it back out of the existing
       -- mmu_regs_o bundle. mmu_regs_o is driven from this_r.mmu
       -- (the REGISTERED CSRs), so that read is a flop output and adds no
       -- loop -- and it costs no new 32-bit port.
       ...
       -- TLB install counters, same pass-through arrangement as the
       -- walker counters above, read via P4_TLBINST. They count slots
       -- ACTUALLY written, so a speculative I->D shadow fill that finds
       -- the page already resident is absent from both -- which is the
       -- property guard mmudshadow exists to observe.
...
signal manip_sel : std_logic_vector(31 downto 0);
 ...
 -- Combinational form of the P4 privilege violation, driven from INSIDE the
 -- datapath process at the refusal statement itself (same technique, and for
 -- the same reason, as ma_launch above: the launch condition tests
 -- this.data_o.en as a mid-evaluation VARIABLE and cannot be re-expressed as
 -- a concurrent assignment). g_p4_viol below flops these onto the p4_viol_o /
 -- p4_viol_wr_o ports; see their declaration for why the flop is required.
 -- Defaulted unconditionally at the top of the process, so on J1/J2 both are
 -- constant '0' with no consumer and prune away.
 signal p4_viol_c : std_logic;
 signal p4_viol_wr_c : std_logic;
 -- Combinational form of the PMU write, driven from inside the process for
 -- the same reason as p4_viol_c above: the write arm is selected by
 -- mid-evaluation variables (p4_serve_v, p4_sel_v, ma_dw) that no concurrent
 -- assignment can re-express. Defaulted unconditionally at the top of the
...
-- setup arithmetic inputs function
 ...
 datapath : process(this_r,pc_ctrl,wbus,zbus,sr_ctrl, xbus, ybus, mac,mem, instr, db_i, inst_i, debug, debug_i,reg_wr_data_o, logic_out, arith_out, arith_func, func, sfto, coproc, cop_i, shift_busy, mult_stall, tlb_exc_pend, squash_arm, tlb_fault_va, tlb_exc_expevt, tlb_exc_fsr, reg, num_x_r, mem_autoupd, mem_autoinc1, mem_predec, restore_fire, delay_slot, inst_fault, inst_fault_prot, id_delay_slot, if_fault_cap, tlb_exc_is_i, tlb_exc_ifetch, ex_if_pc, tlb_squash_r, squash_ifetch_r, pmu_i)
   variable this : datapath_reg_t;
          ...
          -- Which PMU counter a P4_PMCNT access selects. Carried alongside
          -- p4_sel_v rather than folded into it so eight counters cost one
          -- enum member and one 3-bit variable, not eight enum members and
          -- eight decode arms. Defaulted every visit for the same
          -- read-before-write reason p4_sel_v is.
          variable pmu_idx_v : natural range 0 to PMU_NUM_CNT - 1;
          -- SINGLE SOURCE OF TRUTH for "this access is a P4 CSR access the
          -- privilege gate REFUSES / SERVES" (J4+PRIV_ARCH). THREE sites must
          -- agree on this one invariant -- the refusal arm, the serve arm, and
          -- the restart-shadow condition, which needs it INVERTED -- and three
          -- hand-written copies of a predicate (one of them negated) is the
          -- silent-regression hazard squash_arm was factored out to avoid.
          -- Defaulted every visit for the same read-before-write reason
          -- p4_sel_v is (see its default below).
          variable p4_refuse_v, p4_serve_v : boolean;
        ...
        begin
          ...
          ma_launch <= '0';
          -- Defaults for the P4-privilege-violation export (see its
          -- declaration); overridden at the single refusal statement below.
          p4_viol_c <= '0';
          p4_viol_wr_c <= '0';
          p4_refuse_v := false;
          p4_serve_v := false;
          -- Default for the PMU write export. The strobe is overridden ONLY
          -- inside the SERVED P4 write arm, so a user-mode P4 store -- which
          -- takes the REFUSE arm above it -- can never reach a counter. The
          -- refusal arm has no read side effect either (unlike TSBVICT), so a
          -- refused PMU access leaves no trace at all.
          pmu_wr_c <= PERF_WR_ZERO;
          pmu_idx_v := 0;
          ...
          if PRIV_ARCH and tlb_exc_pend = '1' and tlb_exc_is_i = '0'
            ...
            -- is unknown at miss time and is resolved by the walker's install, which
            ...
            -- alu_y=4, so SPC = tlb_exc_pc - 4. The resume (RTE) re-enters
            ...
            -- 0x1054):
            -- * the resume (RTE) re-enters exactly AT SPC, NOT one
          ...
          if this.slot = '1' then
            ...
            if (mem.issue = '1' and this.data_o.en = '0'
              ...
              if PRIV_ARCH then
                ...
                p4_sel_v := P4_NONE;
                -- Derive the gate predicate ONCE, here, from the freshly
                -- decoded segment and the committed SR. Everything below reads
                -- these, never seg_v/sr.md again.
                p4_refuse_v := seg_v = SEG_P4 and this.sr.md = '0';
                p4_serve_v := seg_v = SEG_P4 and this.sr.md = '1';
                -- P4 BLOCK DECODE, PAGE FIRST.
                --
                -- `seg_decode` calls the whole of 0xFF------ P4, and this
                -- core now puts more than one register block in it. The
                -- outer selector below is the 4 KB PAGE
                -- (docs/soc/p4-mmio-map.md 3 allocates P4 per-CPU blocks on
                -- 4 KB boundaries); each arm then decodes offsets WITHIN its
                -- own page and nowhere else.
                --
                -- ADDING A THIRD BLOCK: add a `when x"00N" =>` arm here. Do
                -- NOT append to the MMU chain in the `others` arm -- those
                -- tests match on ma_ad(7 downto 0) ALONE, so an offset added
                -- there answers at that offset in EVERY page, and a new
                -- block's own page would be shadowed at every offset that
                -- collides with an MMU register. It would fail silently, too:
                -- an undecoded P4 offset raises nothing and reads as zero
                -- (p4-mmio-map.md, "Undecoded P4 offsets fail SILENTLY"), so
                -- the new block would simply appear to be permanently
                -- disabled. This structure exists so that hazard cannot be
                -- reached by appending in the obvious place.
                --
                -- The MMU page is the `others` arm rather than `when x"000"`
                -- ON PURPOSE, and it is not tidiness: it keeps the
                -- pre-existing behaviour that the MMU registers answer across
                -- the rest of the P4 window byte-for-byte. That aliasing is
                -- old, undocumented and out of scope to change here; making
                -- it `when x"000"` would silently retire it.
                case ma_ad(23 downto 12) is
                  when x"001" =>
                  ...
                  -- it.
                    ...
                    -- block ever grows programmable event select. All 12
                    -- offset bits are compared, so 0x020..0x03C is the ONLY
                    -- window that answers -- guard pmucnt check 0x04.
                    -- The window and the index slice are DERIVED from
                    -- perf_pkg's pmu_cnt_tag / pmu_cnt_idx_bits rather than
                    -- spelled out here, so the literal lives in ONE file and
                    -- the two slices cannot be edited apart. With
                    -- pmu_cnt_idx_bits = 3 this reads exactly
                  ...
                  -- TSBSLOT: write a VA, read the same address back to get
                  -- tsb_ptr(VA). Read/write like TSBBR/TSBCFG above; the read
                  -- evaluates tsb_ptr() on the latched VA so the TSB index
                  -- function has exactly one implementation. Linux's
                  ...
                  -- 0x58 TLBINST: read-only, [31:16]=ITLB slot writes
                  -- [15:0]=DTLB slot writes. Anti-vacuity instrumentation for
                  -- the I->D shadow fill; not architectural.
                  elsif ma_ad(7 downto 0) = x"58" then p4_sel_v := P4_TLBINST;
                  end if;
                end case;
              end if;
              -- PRIVILEGE GATE. P4 is the SH-4 privileged control-register
              -- segment: "Accesses are untranslated (no MMU lookup), uncached,
              -- and accessible only with SR.MD = 1"
              -- (docs/soc/p4-mmio-map.md §1). NOTHING else enforces that here
              -- -- P4 is not TLB-translated (cpu.vhd asserts d_at_translated
              -- only for SEG_P0/SEG_P3), so no protection bit can intervene,
              -- and the block below is the whole implementation of every MMU
              -- CSR. Without this arm a user-mode `MOV.L Rm,@Rn` to
              -- 0xFF000010/0xFF000014 wrote MMUCR (disabling translation) or
              -- TSBBR (repointing the hardware TSB walker at attacker memory)
              -- -- a complete user->supervisor escape. Guard: mmup4priv.
              if PRIV_ARCH and p4_refuse_v then
                -- REFUSE. No CSR is written and no read-side effect runs --
                -- TSBVICT's LFSR step included, which is why the refusal is a
                -- separate arm ahead of the decode rather than a mask applied
                -- inside it. A refused load completes with a hard zero rather
                -- than stalling: the exception raised one cycle from now
                -- squashes the writeback (squash_arm, via tlb_exc_pend), and
                -- zero rather than stale/privileged data is what the register
                -- would see if that squash were ever defeated.
                if mem.wr = '0' then
                  this.m_dr_next := (others => '0');
                  this.m_en := '1';
                end if;
                -- TEA <- the refused VA, written HERE because this is the only
                -- point that has it. cpu.vhd reads it back through mmu_regs_o
                -- as this fault's VA (see the p4_viol_o declaration), and the
                -- tlb_exc_pend capture block one cycle later rewrites TEA with
                -- the identical value -- so this is idempotent, not a race.
                this.mmu.tea := ma_ad;
                p4_viol_c <= '1';
                p4_viol_wr_c <= mem.wr;
              elsif PRIV_ARCH and p4_serve_v then
                if mem.wr = '1' then
                  case p4_sel_v is
                    when P4_MMUCR =>
                    ...
                    -- PMU writes. Counters ARE writable, and that is a design
                    -- decision rather than an oversight: Linux perf programs a
                    -- sampling period by preloading a counter, and -- decisively
                    -- for this repo -- it is the only way a guard can reach a
                    -- 32-bit counter's wrap inside a simulation stop-time.
              ...
              --
              -- Written for the ordinary bus path AND for a REFUSED user-mode
              -- P4 access -- i.e. for everything except a SERVED P4 CSR access,
              -- which is why this reads `not p4_serve_v` rather than simply
              -- `PRIV_ARCH`. A refused access faults one cycle later,
              -- so it needs its restart context captured exactly like any
              -- other faulting access; leaving the shadow untouched would let
              -- the capture block read a PREVIOUS access's ma_dslot (wrong
              -- restart-PC arm) and ma_autoupd/ma_predec (a bogus base
              -- restore writing the P4 VA into an unrelated GPR) -- the same
              -- stale-shadow hazard documented on the tlb_exc_ifetch gate in
              -- the capture block. A SERVED P4 CSR access is excluded because
              -- it cannot fault, which is how it has always behaved.
              if PRIV_ARCH and not p4_serve_v then
 ...
 -- Flop the P4-privilege-violation export onto its ports. See the
 -- p4_viol_o port declaration for why this must be registered rather than
 -- passed straight out (combinational loop through cpu.vhd's exception
 -- process, and pipeline alignment with the D-side TLB fault it reuses).
 g_p4_viol : if PRIV_ARCH generate
 begin
   process(clk, rst)
   begin
     if rst = '1' then
       ...
       p4_viol_o <= p4_viol_c;
       p4_viol_wr_o <= p4_viol_wr_c;
     end if;
   end process;
 end generate g_p4_viol;
 -- J1/J2: driven to a constant '0' HERE (not left to the port default), so the
 -- J1/J2 datapath exports a hard constant and no flop is instantiated -- same
 -- shape as g_squash_ifetch_off and the other PRIV_ARCH-only exports.
 g_p4_viol_off : if not PRIV_ARCH generate
   p4_viol_o <= '0';
   p4_viol_wr_o <= '0';
 end generate g_p4_viol_off;
 ...
 -- PMU write side straight out (perf.vhd registers it, so it lands on the
 -- same clock edge as the MMU CSR writes beside it in the serve arm).
 pmu_o <= pmu_wr_c;
 mac_s <= this_r.mac_s;
