...
architecture tb of dcache_check_tb is
  ...
  signal dbus_lock     : std_logic;
  signal dbus_ddrburst : std_logic;
  signal dbus_ack_r    : std_logic;
  ...
  begin return x"D0" & std_logic_vector(to_unsigned(i, 24)); end function init_word;

  function init_mem return mem_t is

    variable m : mem_t;

  begin for i in 0 to MEMW - 1 loop

      m(i) := init_word(i);

    end loop; return m; end function init_mem;

  function widx (
    a : std_logic_vector
  ) return integer is -- word index
...
begin
  ...
  clk125 <= not clk125 after 4 ns when not done else
            '0';

  g_sc : if not DUAL_CLOCK generate clk200 <= clk125;
  end generate g_sc;

  g_dc : if DUAL_CLOCK generate
    clk200 <= not clk200 after 2.5 ns when not done else
              '0';
  end generate g_dc;
  ...
  ddr_ackgen : process (dbus_o, dbus_o_d5) is
  begin
  ...
  ddr_rd : process (dbus_o, ddr_rdy) is
  begin

    if (ddr_rdy = '1') then
      dbus_i.d <= ddr_mem(widx(dbus_o.a));
    else
      dbus_i.d <= (others => '0');
    end if;
 ...
 := dbus_o.d(8 * i + 7 downto 8 * i);
  ...
  begin
    ...
    wait for 900 us;
    assert done
      report "WATCHDOG: testbench did not finish"
      severity failure;
    wait;
  ...
  stim : process is
    ...
    procedure tick is begin wait until rising_edge(clk125); end procedure tick;
    ...
    begin
      ...
      loop
        ...
        tick; cyc := cyc + 1; exit when ibus_i.ack = '1';
      ...
      ibus_o <= NULL_DATA_O;
      testno := testno + 1;
      report "LATENCY load @" & hex(addr) & " = " & integer'image(cyc) & " cycles"
        severity note;
    ...
    end procedure do_store;

    procedure chk_load (
      addr : std_logic_vector(31 downto 0)
    ) is
    begin do_load(addr, ref_mem(widx(addr))); end procedure chk_load;

    procedure do_snoop_inval (
      addr : std_logic_vector(31 downto 0)
    ) is
    begin
      ...
      end loop;                                                   -- let the invalidation settle

    end procedure do_snoop_inval;
  ...
  begin
    ...
    report "dcache_check_tb: " & integer'image(testno) & " tests, " &
      ...
      severity note;
    assert errors = 0
      report "DCACHE SCOREBOARD FAILED"
      severity failure;
