...
begin
  ...
  p0 : process (this_r, rst_i, cpa) is
  ...
  begin
    ...
    if (cpa.stallcp = '1') then
      ...
      nx.sbu_num_ex := '0' & cpa.rna;
    end if;

    if (cpa.stallcp = '1') then
      nx.sbu_rnum_ma := this.sbu_rnum_ma;
    else
      nx.sbu_rnum_ma := this.sbu_num_ex;
    end if;
    ...
    if (cpa.stallcp = '1') then
      nx.sbu_oplds_ma := this.sbu_oplds_ma;
    ...
    if ((cpa.stallcp = '0') and (this.sbu_wnum_ma(4) = '1')) then
      if (this.sbu_wnum_ma(3 downto 0) = x"9") then
        if (this.sbu_oplds_ma = '1') then
          ...
          nx.sbu_regfile(9) := this.sbu_regfile(to_integer(unsigned(this.sbu_rnum_ma(2 downto 0))));
        -- CLDS update
        end if;
      else
  ...
  p0_r0 : process (clk_sys, rst_i) is
  begin
