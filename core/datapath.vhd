...
g_div_off : if not SH2A_ARCH generate
 ...
 --
 -- FAULT-SIDE ASYMMETRY WITH z_grace (g_restore2, below). z_grace carries
 -- an explicit `tlb_exc_ifetch and not delay_slot' discriminator with a
 -- long derivation (see that generate's comment). wb_grace carries NO
 -- fault-side term at all -- it is `reg.wr_w and not slot_o' -- and the
 -- "WHY ONE BIT IS ENOUGH" argument above is a purely D-side argument
 -- (only one memory transaction can be outstanding) that does not by
 -- itself establish anything about an I-fetch fault, where no memory
 -- transaction is involved at all. This is believed benign, REASONING
 -- ONLY, not tested: the w-port only ever carries load RESULT data, and
 -- if an I-side fault's squash window admits a stray extra w-port commit
 -- from a re-executed delay-slot instruction, that instruction re-executes
 -- and re-presents the SAME result to the SAME register -- an idempotent
 -- rewrite, not a corruption, unlike the z-port case z_grace exists to
 -- fix (which can drop a writeback entirely, or admit one from the wrong
 -- register). No guard specifically exercises an I-fetch fault with a
 -- genuinely pending w-port writeback whose delay slot is re-executed, so
 -- this paragraph documents the gap rather than closing it -- matching
 -- the honesty of z_grace's own COVERAGE WARNING above. Do not read a
 -- green suite as evidence this asymmetry is safe.
 ...
 g_restore2 : if MMU_ARCH generate
   ...
   -- (mmudrain leg D, at 19890000000 ps: tlb_squash_r=1, tlb_exc_ifetch=1,
   ...
   --
   -- WHY THE DISCRIMINATOR IS EXACTLY (tlb_exc_ifetch AND NOT delay_slot).
   -- Two cases share this machinery and need OPPOSITE handling. This USED
   -- to be phrased as "the same predicate the restart-PC arm already
   -- uses" (tlb_exc_is_i) -- that was the bug: tlb_exc_is_i is narrowed to
   -- IMISS/IPROT for restart-PC selection and is '0' for an I-side
   -- MULTI_HIT, which silently fell into the D-side "no grace" case below
   -- and reproduced the Defect-6 lost-writeback on that fault kind. The
   -- gate now uses tlb_exc_ifetch, which is true for every I-side
   -- exc_kind (IMISS/IPROT/MULTI_HIT); tlb_exc_is_i remains reserved for
   -- picking the restart-PC derivation only. The case analysis below is
   -- otherwise unchanged:
   -- * D-side fault (tlb_exc_ifetch='0'), including one taken BY a
   -- delay-slot instruction: the restart is at the BRANCH (or at the
   -- faulting instruction itself), so the pending write WILL be
   -- re-executed and MUST be squashed. No grace. This is case (a).
   ...
   -- only. Dropping the term (z_grace <= tlb_exc_ifetch) leaves all 68
   ...
   -- of a green suite.
   -- (The tlb_exc_ifetch half IS proven load-bearing in both directions:
   -- forcing z_grace to '0' fails mmudrain leg D, and forcing it to '1'
   -- -- i.e. granting grace on the D-side too -- fails leg A. It is
   -- further exercised on the I-side MULTI_HIT guard added for this
   -- fix, see m8_multihit_ifetch below.)
   -- * I-side fault whose restart PC IS the faulting fetch VA
   -- (tlb_exc_ifetch='1', delay_slot='0'): nothing younger than the faulting
 ...
 begin
   ...
   begin
     if rst = '1' then
       ...
       if tlb_squash_r = '0' and squash_arm = '1' then
         z_grace <= tlb_exc_ifetch and not delay_slot;
       elsif slot_o = '1' then
         z_grace <= '0';
       end if;
     end if;
