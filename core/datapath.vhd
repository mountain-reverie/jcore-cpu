...
entity datapath is
 ...
 port (
       ...
       -- Per-instruction fetch-PC round-trip (J4). if_pc = the VA of
   ...
   variable this : datapath_reg_t;
        ...
        begin
          ...
          if PRIV_ARCH and tlb_exc_pend = '1' and this.tlb_exc_captured = '0' then
            ...
            --
            -- Gated on tlb_exc_is_i='0': arm the restore for D-SIDE faults ONLY.
            -- ma_numz / ma_autoupd / ma_predec are written only at the
            -- memory-access pipeline point and are never cleared on a non-memory
            -- instruction, so on an INSTRUCTION-FETCH fault (IMISS/IPROT) they
            -- still hold the last post-increment load's base register and a
            -- stale ma_autoupd='1'. Un-gated, that armed a bogus restore and
            -- restore_fire wrote the I-side faulting FETCH VA into that GPR.
            -- Both branches must be gated: a pre-decrement store shadow is
            -- equally stale on an I-side fault.
            if tlb_exc_is_i = '1' then
