...
package mult_pkg is
  ...
  type mult_reg_t is record
    ...
    acc_sq : std_logic;
  end record mult_reg_t;

  constant mult_reset : mult_reg_t :=
  (
    state     => NOP,
    result_op => IDENTITY,
    m1        => (others => '0'),
  ...
  end record mult_decode_t;

  type mult_macout_t is record
    mach : std_logic_vector(31 downto 0);
    macl : std_logic_vector(31 downto 0);
  end record mult_macout_t;
  ...
  function mult_finalize (
    dec : mult_decode_t;
    prod_mag : unsigned(63 downto 0);
    cur_mach,
    cur_macl : std_logic_vector(31 downto 0)
  )
...
package body mult_pkg is
  ...
  begin
    ...
    if (command = MACL) then
      if (s = '1') then
        d.result_op := SATURATE64;
      end if;
    elsif (command = MACW) then
      if (s = '1') then
        d.result_op := SATURATE32; cmd := MACWS;
      end if;
    end if;
    ...
    case cmd is
      ...
      when MULL =>
        ...
        d.width := 32; signd := '1'; d.macl_en := '1'; d.use_h := '0';

      when MULSW =>

        d.width := 16; signd := '1'; d.macl_en := '1'; d.use_h := '0';

      when MULUW =>

        d.width := 16; signd := '0'; d.macl_en := '1'; d.use_h := '0';

      when DMULSL =>

        d.width := 32; signd := '1'; d.mach_en := '1'; d.macl_en := '1';

      when DMULUL =>

        d.width := 32; signd := '0'; d.mach_en := '1'; d.macl_en := '1';
    ...
    if (signd = '1') then
      if (d.width = 32) then
        if (opa(31) = '1') then
          a_neg := '1'; mcand0  := unsigned(-signed(mcand0));
        end if;
        if (m2(31) = '1') then
          b_neg := '1'; mplier0 := unsigned(-signed(mplier0));
        end if;
      else
        if (opa(15) = '1') then
          a_neg := '1'; mcand0  := x"0000" & unsigned(-signed(mcand0(15 downto 0)));
        end if;
        if (m2(15) = '1') then
          b_neg := '1'; mplier0 := x"0000" & unsigned(-signed(mplier0(15 downto 0)));
        end if;
      end if;
    end if;
    ...
    if (command = DMULSL or command = DMULUL) then
      d.clr_mach := '1';
    end if;

    d.clr_macl := '0';

    if (command /= NOP and command /= MACL and command /= MACW) then
      d.clr_macl := '1';
    end if;

    return d;

  end function mult_decode;
  ...
  end function mult_finalize;

end package body mult_pkg;
