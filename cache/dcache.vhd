...
begin
  ...
  udcache_mcl : component dcache_mcl
    port map (
      clk => clk200,
      -- ------
      rst    => rst,
  ...
  c2 : process (thisbm_r,    thisbc_r,    en0mcl_en,   en0mcl_data, en0ccl,
                ...
                en3ccl,      bcen_value_halfcb0,       bmen_value_halfcb2) is
  ...
  begin
    ...
    if (en0mcl_en = '1') then
      thisbm.en0(VALUE) := bcen_value_halfcb0;
    end if;
    ...
    if (thisbc.en2(METASTABLE) = '1') then
      ...
      thisbc.d2s(METASTABLE) := thisbm.d2v;
    end if;
    ...
    -- enable and data flacters ------------------------------------------------
    thisbm.en30(METASTABLE) := thisbm_r.en30(VALUE) xor not thisbc_r.en30(VALUE);
    thisbc.en30(METASTABLE) := thisbm_r.en30(VALUE) xor     thisbc_r.en30(VALUE);
    -- set the enable flops and latch the input data ---------------------------
    if (en3mcl(0) = '1') then
      thisbm.en30(VALUE) :=     thisbc_r.en30(VALUE);
    end if;

    if (en3ccl(0) = '1') then
      thisbc.en30(VALUE) := not thisbm_r.en30(VALUE);
    end if;
    ...
    -- enable and data flacters ------------------------------------------------
    thisbm.en31(METASTABLE) := thisbm_r.en31(VALUE) xor not thisbc_r.en31(VALUE);
    thisbc.en31(METASTABLE) := thisbm_r.en31(VALUE) xor     thisbc_r.en31(VALUE);
    -- set the enable flops and latch the input data ---------------------------
    if (en3mcl(1) = '1') then
      thisbm.en31(VALUE) :=     thisbc_r.en31(VALUE);
    end if;

    if (en3ccl(1) = '1') then
      thisbc.en31(VALUE) := not thisbm_r.en31(VALUE);
    end if;
  ...
  c2_r2_latch : if not CACHE_SAME_CLOCK generate

    c2_r2 : process (clk200, thisbm_r) is -- transparent latch 0.5 cycle delay
    begin
  ...
  end generate c2_r2_latch;

  c2_r2_ff : if CACHE_SAME_CLOCK generate

    c2_r2 : process (clk200) is            -- full-cycle (posedge FF, single-clock):
  ...
  end generate c2_r2_ff;

  c2_r3_latch : if not CACHE_SAME_CLOCK generate

    c2_r3 : process (clk125, thisbc_r) is -- transparent latch 0.5 cycle delay
    begin
  ...
  end generate c2_r3_latch;

  c2_r3_ff : if CACHE_SAME_CLOCK generate

    c2_r3 : process (clk125) is            -- full-cycle (posedge FF, single-clock):
