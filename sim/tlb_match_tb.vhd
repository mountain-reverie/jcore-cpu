...
begin
  ...
  begin
    ...
    wait for 1 ns;
    assert i_hit = '1'
      report "T1: miss on installed entry"
      severity failure;
    assert i_prot = '0'
      report "T1: spurious prot on readable+executable entry"
      severity failure;
    -- pa_tag = ppn1(27 downto 13) = PA[27:13] = 15 bits (28-bit cache region)
    assert i_pa_tag = ppn1(27 downto 13)
      report "T1: wrong PA tag"
      severity failure;
    assert i_c = '1'
      report "T1: i_c should reflect installed C=1 bit"
      severity failure;
    ...
    wait for 1 ns;
    assert i_hit = '1'
      report "T1b: miss on installed C=0 entry"
      severity failure;
    assert i_c = '0'
      report "T1b: i_c should be 0 for C=0 entry"
      severity failure;
    ...
    wait for 1 ns;
    assert i_hit = '0'
      report "T2: false hit on VPN mismatch"
      severity failure;
    ...
    wait for 1 ns;
    assert i_hit = '0'
      report "T3: false hit on ASID mismatch"
      severity failure;
    asid <= x"0001";
    ...
    wait for 1 ns;
    assert i_hit = '1'
      report "T4: global entry missed on ASID mismatch"
      severity failure;
    asid <= x"0001";
    ...
    wait for 1 ns;
    assert i_hit = '1'
      report "T5: miss when should hit"
      severity failure;
    assert i_prot = '1'
      report "T5: i_prot not raised for X=0"
      severity failure;
    ...
    assert d_hit = '1'
      ...
      severity failure;
    assert d_prot = '1'
      report "T6: d_prot not raised for W=0 on write"
      severity failure;
    d_we <= '0';
    ...
    assert i_pa_tag /= d_pa_tag
      report "T8: same PA tag for different entries"
      severity failure;

    report "tlb_match_tb: all tests passed"
      severity note;
    wait;
