...
architecture arch of decode is
    ...
    -- Ordered exception delivery (J4+MMU_ARCH). '1' while an instruction OLDER
    -- than the one in ID is un-retired, i.e. it can still raise a synchronous
    -- exception. Three terms: one combinational (dispatch cycle), two registered
    -- (history):
    --   ex_stall.ma_issue         ID-stage combinational dispatch signal: a memory
    --                             access that is being dispatched THIS cycle (not
    --                             yet in pipeline_r). Needed because without it the
    --                             predicate would assert one cycle LATE (550ns vs
    --                             540ns in mmufaultage), making Task 3's exception
    --                             gating a no-op.
    --   pipeline_r.ex1_stall.ma_issue  EX-stage: a memory access dispatched last
    --                             cycle but not yet fault-resolved
    --   pipeline_r.wb*_stall.wrreg_w   WB1/2/3-stage: a w-port writeback still
    --                             draining (the w-port retires a slot late, so an
    --                             instruction whose w-port write has not landed has
    --                             NOT retired)
    -- Registered, never combinational: its consumer is the next_op mux, which is
    -- Fmax-critical. See docs/superpowers/specs/2026-08-06-mmu-ordered-exception-
    -- delivery-design.md.
    signal older_unretired : std_logic;
...
begin
    ...
    core : decode_core
        port map (
            ...
            older_unretired => older_unretired,
            if_pc => if_pc,
            if_pc_o => if_pc_c
        );
    table : decode_table
    ...
    g_older : if MMU_ARCH generate
      process(clk, rst)
      begin
        if rst = '1' then
          older_unretired <= '0';
        elsif clk = '1' and clk'event then
          older_unretired <= ex_stall.ma_issue
                             or pipeline_r.ex1_stall.ma_issue
                             or pipeline_r.wb1_stall.wrreg_w
                             or pipeline_r.wb2_stall.wrreg_w
                             or pipeline_r.wb3_stall.wrreg_w;
        end if;
      end process;
    end generate g_older;
    g_older_off : if not MMU_ARCH generate
      older_unretired <= '0';
    end generate g_older_off;
    ...
    sr.mmu_reg_wr  <= pipeline_r.ex1_stall.mmu_reg_wr;
    sr.mmu_reg_sel <= pipeline_r.ex1_stall.mmu_reg_sel;
    sr.tlb_wr      <= pipeline_r.ex1_stall.tlb_wr;
