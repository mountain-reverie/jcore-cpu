...
architecture beh of dcache_ccl is
  signal this_c : dcacheccl_reg_t;
  signal this_r : dcacheccl_reg_t := DCACHECCLK_REG_RESET;
...
begin
  p0 : process(ra,a,a_lock,sa,mtoc,ctrl,this_r)
    variable this : dcacheccl_reg_t;
  -- ---------------------------------------------------------------------------
  -- variables
  -- ---------------------------------------------------------------------------
  ...
  variable tag_to_compare : std_logic_vector(CACHE_PA_TAG_WIDTH-1 downto 0);
  ...
  begin -- begin of process ( )
    ...
    if(nx.sabank_rcv = '0') then
         sy.al <= this_r.saout_al2a;
    else sy.al <= this_r.saout_al2b; end if;
    ...
    if MMU_ARCH and this.a_prev_mmu.at = '1' then
      tag_to_compare := this.a_prev_mmu.pa_tag;
    else
      tag_to_compare := (CACHE_PA_TAG_WIDTH-CACHE_TAG_WIDTH-1 downto 0 => '0')
                        & to_cache_tag(this.a_prev.a);
    end if;
    if(((this.aen_del1 = '1') and (ra.tag0 = tag_to_compare)) or
    ...
    -- ------------------------------------------------------------------------
    -- FIXME(MMU_ARCH): snoop compare uses VA-based tag (15 bits). When AT=1,
    -- stored tags are PA-based (15 bits); a VA snoop will not match.
    -- Full snoop correctness under translation requires PA-based invalidation
    -- (planned for a later MMU milestone, not M2 scope).
    if(this.sa_al(CACHE_REGION_WIDTH-CACHE_LINE_WIDTH_BITS-1 downto
                  CACHE_INDEX_BITS) = ra.tag1(CACHE_TAG_WIDTH-1 downto 0) ) and
    ...
    if(a.en = '1') then nx.a_prev := a;
                                 nx.a_prev_mmu := a_mmu;
    else nx.a_prev := this.a_prev;
                                 nx.a_prev_mmu := this.a_prev_mmu; end if;
    if(nx.state = RMISS1) or
      (nx.state = WMISS1) or
      (nx.state = RLOCK1) then nx.ma0 := a.a(27 downto 0);
                                 nx.ma0_at := a_mmu.at;
                                 nx.ma0_pa_tag := a_mmu.pa_tag;
    else nx.ma0 := this.ma0;
                                 nx.ma0_at := this.ma0_at;
                                 nx.ma0_pa_tag := this.ma0_pa_tag; end if;
    ...
    this.ma0 := nx.ma0;
    this.ma0_at := nx.ma0_at;
    this.ma0_pa_tag := nx.ma0_pa_tag;
    this.a_prev := nx.a_prev;
    this.a_prev_v := nx.a_prev_v;
    this.a_prev_mmu := nx.a_prev_mmu;
...
this_c <= this;
  ...
  ry_ccl.ten0 <= '1';
  ry_ccl.tag0 <= this_r.ma0_pa_tag when MMU_ARCH and this_r.ma0_at = '1'
                 else (CACHE_PA_TAG_WIDTH-CACHE_TAG_WIDTH-1 downto 0 => '0')
                      & this_r.ma0 (27 downto CACHE_LINE_WIDTH_BITS+CACHE_INDEX_BITS);
end beh;
