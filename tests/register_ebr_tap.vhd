...
architecture tb of register_ebr_tap is
  ...
  signal addr_ra   : std_logic_vector(4 downto 0) := (others => '0');
  signal addr_rb   : std_logic_vector(4 downto 0) := (others => '0');
  signal w_addr_wb : std_logic_vector(4 downto 0) := (others => '0');
  signal w_addr_ex : std_logic_vector(4 downto 0) := (others => '0');
  ...
  signal addr_ra_early, addr_rb_early : std_logic_vector(4 downto 0)  := (others => '0');
  ...
  signal a_e : std_logic_vector(31 downto 0);
  signal b_e : std_logic_vector(31 downto 0);
  signal z_e : std_logic_vector(31 downto 0);
  signal a_f : std_logic_vector(31 downto 0);
  signal b_f : std_logic_vector(31 downto 0);
  signal z_f : std_logic_vector(31 downto 0);
  ...
  procedure check (
    signal ae, af, be, bf, ze, zf : in std_logic_vector(31 downto 0);
    desc                          : string
  ) is
  begin
...
begin
  ...
  dut : entity work.register_file(ebr)
    generic map (
      addr_width => 5, num_regs => 16, reg_width => 32
    )
    port map (
      clk           => clk,
      rst           => rst,
      ...
      addr_rb       => addr_rb,
      dout_b        => b_e,
      addr_ra_early => addr_ra_early,
      addr_rb_early => addr_rb_early,
      dout_0        => z_e,
      we_wb         => we_wb,
      w_addr_wb     => w_addr_wb,
      din_wb        => din_wb,
  ...
  begin
    ...
    addr_ra_early <= S(0).ra;
    addr_rb_early <= S(0).rb;

    for i in s'range loop

      wait until rising_edge(clk);                                             -- ebr q latches reg[S(i).ra]
      addr_ra   <= S(i).ra;
      addr_rb   <= S(i).rb;
      we_wb     <= S(i).ewb;
      w_addr_wb <= S(i).awb;
      ...
      if (i < s'high) then
        addr_ra_early <= S(i + 1).ra;
        addr_rb_early <= S(i + 1).rb;
      end if;

      wait until falling_edge(clk);
      wait for 1 ns;                                                           -- let ebr's q_a/q_b settle
    ...
    we_ex         <= '1';
    w_addr_ex     <= "00001";
    din_ex        <= x"00000000";                                              -- must not commit
