...
begin
  ...
  -- the cache idle (en=0).
  c_cpu_o <= cpu_o when cacheable else NULL_DATA_O;

  u_dcache : entity work.dcache_adapter
    port map (
      clk125 => clk125, clk200 => clk200, rst => rst, ctrl => ctrl,
      ibus_o => c_cpu_o, a_mmu => a_mmu, lock => lock, ibus_i => c_cpu_i,
      snpc_o => snp_o, snpc_i => NULL_SNOOP_IO,
      dbus_o => c_mem_o, dbus_lock => c_mem_lock, dbus_ddrburst => c_mem_ddrb,
  ...
  mem_o        <= c_mem_o    when cacheable else cpu_o;
  mem_lock     <= c_mem_lock when cacheable else lock;
  mem_ddrburst <= c_mem_ddrb when cacheable else '0';

  -- CPU response mux.
  cpu_i <= c_cpu_i when cacheable else mem_i;
end architecture;
