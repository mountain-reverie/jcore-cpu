...
architecture beh of icache_mcl is
  signal this_c : icachemcl_reg_t;
  signal this_r : icachemcl_reg_t := ICACHEMCLK_REG_RESET;
begin
  p0 : process(ma,ctom,this_r)
    variable this : icachemcl_reg_t;
  ...
  my.we <= x"0";
  my.d <= x"00000000";
  my.ddrburst <= this_r.mav and this_r.ic_onm;
  -- output signal (cache ram port)
  ry_mcl.a1 <= this_r.ma0(CACHE_LINE_WIDTH_BITS+CACHE_INDEX_BITS-1 downto 2);
  ry_mcl.en1 <= this_r.men and ma.ack_r;
  ry_mcl.wr1 <= this_r.men and ma.ack_r;
  ry_mcl.d1 <= ma.d;
  -- output signal (mem-clock domain -> cpu-clock domain,
  -- domain crossing)
  mtoc.v <= this_r.wv;
  mtoc.rfillv <=
    this_r.rfillv when this_r.ic_onm = '1' else
    this_r.rfillv_del1;
  mtoc.rfillv_advance <= this_r.rfillv;
  mtoc.rfilld <= this_r.ic_onm & this_r.d;
  mtoc.cd <= this_r.cd;
end beh;
