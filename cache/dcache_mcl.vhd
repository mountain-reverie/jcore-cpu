...
architecture beh of dcache_mcl is
  signal this_c : dcachemcl_reg_t;
  signal this_r : dcachemcl_reg_t := DCACHEMCLK_REG_RESET;
  signal ctom_cmd : std_logic_vector(2 downto 0);
  signal sbout_cmd_s : std_logic_vector(2 downto 0);
  signal sbout_en1_sig : std_logic;
begin
  p0 : process(ma,ctom,this_r)
    variable this : dcachemcl_reg_t;
...
-- if(nx.dc_onm = '1') and -- to transfer onm same time as cmd
  ...
  ry_mcl.a1 <= this_r.ma0(CACHE_LINE_WIDTH_BITS+CACHE_INDEX_BITS-1
                downto 2);
  ry_mcl.en1 <= this_r.men and (not this_r.mwr) and ma.ack_r;
  ry_mcl.wr1 <= this_r.men and (not this_r.mwr) and ma.ack_r;
  ry_mcl.we1 <= this_r.ry_we1 and
                ( ma.ack_r & ma.ack_r & ma.ack_r & ma.ack_r );
  ry_mcl.d1 <= ma.d;
  my.lock <= this_r.mlock;
  my.ddrburst <= this_r.ddrburst;
  my.a <= this_r.ma0;
  my.av <= this_r.mav;
  my.en <= this_r.men;
  my.wr <= this_r.mwr;
  ...
  -- domain crossing)
  mtoc.b0d_unc <= this_r.b0d_unc;
  mtoc.b2d_cfil <= this_r.cd;
  mtoc.b2enr <= this_r.b2en;
  -- delay critical (ma.ack dependent output)
  mtoc.b30enr <= this_c.b3enr_pls and
                   (not this_c.b3enr_dir);
  mtoc.b31enr <= this_c.b3enr_pls and
                        this_c.b3enr_dir ;
end beh;
