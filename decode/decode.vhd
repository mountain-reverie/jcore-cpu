...
architecture arch of decode is
    ...
    -- exception. Two terms, both already carried in pipeline_r:
    --   ex1_stall.ma_issue  a memory access is dispatched but not fault-resolved
    --   wb*_stall.wrreg_w   a w-port writeback is still draining (the w-port
    --                       retires a slot late, so an instruction whose w-port
    --                       write has not landed has NOT retired)
...
begin
    ...
    -- Compute ordered_unretired unconditionally here; decode_core gates it with
    -- MMU_ARCH, so on non-MMU builds the synthesis tool optimizes away the logic.
    -- Combine both combinational (ex_stall) and registered (pipeline_r) terms to
    -- capture dispatch signals in their dispatch cycle rather than the next.
    process(clk, rst)
    begin
      if rst = '1' then
        older_unretired <= '0';
