...
//===----------------------------------------------------------------------===//
//
// Automatically generated by tools/insns2asm from docs/insns.json. DO NOT EDIT.
//
//===----------------------------------------------------------------------===//
...
// emitFixedBits writes coalesced constant-bit assignments to b.
// Adjacent runs of constant bits {hi..lo} are collapsed to a single
// `let Inst{hi-lo} = 0b<MSB-first>;` line; lone bits use `let Inst{N} = v;`.
func emitFixedBits(b *strings.Builder, bits []struct{ pos, val int }) {
	type run struct{ hi, lo int; vals []int }
	var runs []run
	for _, bv := range bits {
		if len(runs) > 0 && bv.pos == runs[len(runs)-1].lo-1 {
			r := &runs[len(runs)-1]
			r.lo = bv.pos
			r.vals = append(r.vals, bv.val)
		} else {
			runs = append(runs, run{hi: bv.pos, lo: bv.pos, vals: []int{bv.val}})
		}
	}
	for _, r := range runs {
		if len(r.vals) == 1 {
			...
			lit := make([]byte, len(r.vals))
			for i, v := range r.vals {
				lit[i] = byte('0' + v)
			}
			fmt.Fprintf(b, "  let Inst{%d-%d} = 0b%s;\n", r.hi, r.lo, lit)
		}
	}
}
...
func EmitInstrInfo(insns []ir.Insn) string {
	...
	for _, in := range insns {
		...
		// fixed bits (coalesced into contiguous ranges)
		var fixedBits []struct{ pos, val int }
		for wi, w := range in.Words {
			for i := 0; i < 16; i++ {
				...
				fixedBits = append(fixedBits, struct{ pos, val int }{bitPos(wi, i, len(in.Words)), int(bit.Val)})
			}
		}
		emitFixedBits(&b, fixedBits)
