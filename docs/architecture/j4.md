...
| **TLB-fault dispatch** | `decode/decode_core.vhm` under `g_texc : if PRIV_ARCH` | Turns TLB `hit`/`prot` results into SH-4 exception entries: IMISS→`0x040`, DMISS_R→`0x060`, DMISS_W→`0x080`, IPROT→`0x0A0`, DPROT_{R,W}→`0x0C0` (mapped in `core/cpu.vhd`). Exception hold uses `SR.RB`/`SR.BL` to block re-entrancy. |
