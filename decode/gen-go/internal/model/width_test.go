...
func TestBuildBodyAddrSentinel(t *testing.T) {
	empty := func() *Body {
		return BuildBody(map[string]int{}, map[string]logic.LogicMap{}, map[string]bool{}, map[string]bool{}, 8)
	}
	if got := empty().AddrSentinel; got != `x"ff"` {
		t.Errorf("8-bit AddrSentinel = %q, want %q", got, `x"ff"`)
	}
	b9 := BuildBody(map[string]int{}, map[string]logic.LogicMap{}, map[string]bool{}, map[string]bool{}, 9)
