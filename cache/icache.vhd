...
begin
  ...
  uicache_mcl : component icache_mcl
    port map (
      clk => clk200,
      -- ------
      rst    => rst,
  ...
  c2 : process (thisbm_r, thisbc_r, thisuc_r, enmcl, enmcl_for_d, d_mcl, enccl, d_ccl, mtoc1, bcen_value_halfcb0) is
  ...
  begin
    ...
    if (enccl = '1') then
      ...
      thisbc.d (VALUE) := d_ccl;
    end if;

    if (enmcl = '1') then
      thisbm.en(VALUE) := bcen_value_halfcb0;
    end if;

    if (enmcl_for_d = '1') then
      thisbm.d (VALUE) := d_mcl;
    end if;
