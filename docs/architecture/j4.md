...
They are gated by a single entity generic so the core can be elaborated either as
plain J2 or as the full privileged machine from the same sources:

| Generic | Default | When `true` enables |
|---|---|---|
| `PRIV_ARCH` | `false` | SH-4 privilege: `SR.MD/RB/BL`, banked R0–R7, register-model exception entry/return, `EXPEVT`/`INTEVT`/`TRA` capture (`priv_o`), **and** the MMU: the `tlb` CAM, address translation (`AT`), MMU control registers (PTEH/PTEL/ASIDR/MMUCR), TLB-fault exception dispatch, and physical PA tags to the caches (`mmu_o`). There used to be a separate `PRIV_ARCH` generic; it was merged into `PRIV_ARCH` (the two were never set independently — privilege without the MMU was never a real configuration) and no longer exists in the RTL. |

The generic defaults `false`, so the **bare `cpu_j4` configuration is
byte-identical to [J2](j2.md)**. The SH-4 hardware only appears when a
configuration turns the generic on (see the configuration matrix below). This is
...
- **Zero-cost when off.** `PRIV_ARCH=false` elaborates byte-identically to J2;
  every SH-4 addition (privilege AND the MMU) lives inside `if PRIV_ARCH` guards
  or in additive files (`spec/sh4/`, `core/tlb.vhd`).
- **Synthesizable, not just simulatable.** The MMU hardware is `check -assert`-gated
  in the j4c ASIC flow (see *Synthesis* below).

## Configuration matrix

| Configuration | Of | `PRIV_ARCH` | Purpose |
|---|---|---|---|
| `cpu_j4` | `cpu` (sim) | false | Bare J4 == J2; parity build |
| `cpu_sim` | `cpu` (sim) | (decoder `PRIV_ARCH=true`) | Functional sim build; keeps TLB decode logic live |
| `cpu_synth_j4` | `cpu` | false | J4 baseline synth (== J2) |
| `cpu_synth_j4_priv` | `cpu` | **true** | Privileged J4 (banked regs, exceptions) — area flows |
| `cpu_cache_timing_j4_priv_mmu` | `cpu_cache_timing_top` | **true** | **Full J4+MMU+cache** (the `j4c` ASIC build). `PRIV_ARCH` implies the MMU; there is no priv-without-MMU configuration. |

Sources: `core/cpu_config_j4.vhd` (`cpu_j4`), `core/cpu_config_sim.vhd`
(`cpu_sim`), `synth/cpu_synth_j4_config.vhd` (`cpu_synth_j4`,
`cpu_synth_j4_priv`), `synth/cpu_cache_timing_config.vhd`
(`cpu_cache_timing_j4_priv_mmu`).

## Block diagram

Shown with `PRIV_ARCH=true` — the `g_mmu` generate block in `core/cpu.vhd`
instantiates the real `u_tlb` and the TLB-fault → EXPEVT logic. With the generic
`false`, the highlighted blocks are pruned and the core collapses to [J2](j2.md).

```mermaid
flowchart TB
    subgraph CPU["cpu (architecture stru) — cpu_synth_j4 + PRIV_ARCH=true"]
        direction TB
        DEC["u_decode : decode\nbase SH-2 + spec/sh4 overlay\n(decode_core gates TLB-fault dispatch on PRIV_ARCH)"]:::sh4
...
| **MMU CSRs + exception capture** | `core/datapath.vhm` under `if PRIV_ARCH` | P4-MMIO MMU control registers (PTEH/PTEL/ASIDR/MMUCR) and capture of `EXPEVT`/`INTEVT`/`TRA`, exported on `priv_o`; MMU register state exported to the TLB. |
| **`perf`** (`u_perf`) | `core/perf.vhd`, instantiated in `core/cpu.vhd` under `g_tlb_walk : if PRIV_ARCH generate` | **Performance-monitor counters (PMU)**. Eight 32-bit free-running counters — cycles, instruction dispatches, I-fetch references and wait cycles, D-access references and wait cycles, TSB walks and TSB walk hits — plus `PMCR` (bit 0 = EN, resets set), a sticky write-1-to-clear per-counter overflow register `PMOVF`, and an identity register `PMIDR`. Read and written through the P4 page `0xFF001000` (decoded in `core/datapath.vhm`; all 12 offset bits compared, so the page has no aliases). Counters are writable, which is what lets Linux `perf` program a sampling period and what makes a 32-bit wrap reachable inside a simulation stop-time. The TSB walker holds no counters of its own — it exports `ev_walk`/`ev_hit` pulses and `P4_TSBCNT` is served from the low halves of `PMWLK`/`PMWHT`. **See [../pmu/perf-counters.md](../pmu/perf-counters.md) for widths, overflow semantics, the reader protocol and the privilege argument.** |
| **TLB-fault dispatch** | `decode/decode_core.vhm`: `texc_req`/`texc_kind_held` hold register (set by `tlb_exc_en`, cleared by `texc_ack`) plus `PRIV_ARCH and texc_req = '1' and texc_kind_held = ...` conditional arms of the `next_op` signal assignment — inline conditional logic, not a labelled generate block | Turns TLB `hit`/`prot` results into SH-4 exception entries: IMISS→`0x040`, DMISS_R→`0x060`, DMISS_W→`0x080`, IPROT→`0x0A0`, DPROT_{R,W}→`0x0C0` (mapped in `core/cpu.vhd`). Exception hold uses `SR.RB`/`SR.BL` to block re-entrancy. |
| **D-store fault squash** | `core/cpu.vhd` `g_dstore_squash : if PRIV_ARCH` | A store that is itself TLB-faulting is demoted from write to read so it cannot corrupt memory before the fault is taken. |
...
- **`mmu.toml`** — MMU control & TLB ops: `LDC Rm,{PTEH,PTEL,ASIDR}` (the
  write side), **`LDTLB`** (install entry from PTEH/PTEL/ASIDR) and
  **`LDTLB.RN`** (non-delayed atomic load-TLB-and-return, fused for handler
  atomicity). Also pins bit 11 in `Break`/`Reset` debug opcodes so they don't
  alias the TLB exception nibbles `0xA`/`0xB`. There is **no register read
  side**: the CSRs — including read-only **TSBPTR** — are read through their P4
  MMIO aliases. The `0000 nnnn xxxx 1011` family that once held
  `STC {TSBPTR,PTEH,PTEL,ASIDR},Rn`, `CMP/EQ {PTEH,ASIDR},Rn` and
  `LDTLB.RN Rm` was retired with the software TLB-miss hot path those forms
  existed to shorten.
...
| Build | Flat cells | Δ |
|---|---:|---|
| j4c PRIV-only, historical (was `PRIV_ARCH=false`) | 542,091 | — |
| j4c PRIV+MMU (current `cpu_cache_timing_j4_priv_mmu`, `PRIV_ARCH=true`) | 551,106 | **+9,015 (+1.66 %)** |

**The PRIV-only row is historical only.** It was measured before `PRIV_ARCH` was
merged into `PRIV_ARCH`; a priv-without-MMU build (the old M0 timing config,
`cpu_cache_timing_j4_priv`) is **no longer a buildable configuration** — `PRIV_ARCH`
now always brings the MMU with it. The row is kept for the record of what the MMU
itself cost, not as a live build target.
...
## Fmax: the hardware TSB walker, measured

*(Measured 2026-08-11 on ECP5, full rebuilds per leg via `scripts/fmax_ab.sh`,
branch `mmu/tsb-hw-walker`. Implementation status: Phases 1–3 in-branch, not
merged.)*

| point | Fmax | logic | routing | levels | vs prev |
|---|---:|---:|---:|---:|---:|
| pre-walker | 30.34 MHz | | | | — |
| Phase 1 — walker on | 28.65 MHz | | | | −5.6 % |
| Phase 2 — ASID fold + 2-way TSB | 25.82 MHz | 5.5 ns | 33.3 ns | 22 | −9.9 % |
| Phase 3 — seven encodings retired | **28.49 MHz** | 6.4 ns | 28.7 ns | 25 | **+10.3 %** |

Net against pre-walker: **−1.85 MHz (−6.1 %)**, bought for a TLB miss dropping
from **DMISS 23 → 7** and **IMISS 22 → 8** cycles. Phase 2's −9.9 % was never
measured at the time; the A/B that found it also showed Phase 3 repaying very
nearly exactly that amount.

**The mechanism is congestion relief, not logic depth — and that is the part
worth remembering.** Across Phase 2 → Phase 3, routing fell 33.3 → 28.7 ns
while **logic *rose*** 5.5 → 6.4 ns and levels **rose** 22 → 25. Removing seven
decoder encodings did not shorten the logic chain; it un-crowded the die. The
critical path is **82 % routing**, and its single largest item is a **3.7 ns**
hop carrying one MMU register bit from the datapath at `(88,38)` to the TLB at
`(110,9)` — a wire crossing the die, with 15 further hops over 1 ns behind it.

**Consequence for future work:** Fmax effort on this core should go to
**placement and floorplanning of the datapath↔TLB↔icache triangle**, not to
gate-count or logic-depth reduction. A change that removes logic but leaves the
MMU registers far from the TLB will not show up. This is consistent with the
older finding that J4/J2's critical path is decoder *combinational depth* only
in the sense that the decoder is what is being routed across.

J4's privilege + MMU machinery is exercised by a dedicated SH-2-asm regression
suite built with `CONFIG_PRIV_ARCH=1` (this single flag now enables both
privilege and the MMU; there is no separate MMU flag):
...
- **Unit TB:** `sim/tlb_tb.vhd` (27 cases: install/lookup, ASID isolation,
  GLOBAL, STALE, page masks, install dedup, multi-hit, the U/MD and SMEP and W
  protection predicates, warm reset, the C-bit export, and concurrent I/D PA
  distinctness), `sim/tests/tlbwalk.c`. It runs from `sim/mmu_sim.sh` and
  `.github/workflows/full-regression.yml`.
  `sim/tlb_match_tb.vhd` was retired 2026-08-23 — rotted, and mostly redundant.
  The two properties it alone held (the C-bit export and I/D PA distinctness)
  were moved into `tlb_tb` as cases 26 and 27 rather than dropped; see the note
  in `sim/Makefile`.

CI runs the full set in `.github/workflows/full-regression.yml`
(`make -C decode generate-j4` then the `CONFIG_PRIV_ARCH=1` guard loop).
`pr-quick.yml` elaborates all three decoder architectures against the
...
# Functional sim with privilege + MMU enabled (CPU_VARIANT selects the J4
# generics tuple from variants.toml; PRIV_ARCH=true always brings the MMU):
make -C sim CPU_VARIANT=j4 CONFIG_PRIV_ARCH=1   # build cpu_tb
make CONFIG_PRIV_ARCH=1 -C tests mmuxlate.img
...
- Privilege + CSRs + exception capture: `core/datapath.vhm` (`PRIV_ARCH` guards; implies the MMU)
- TLB-fault dispatch: `decode/decode_core.vhm` (`texc_req`/`texc_kind_held` hold register + `PRIV_ARCH`-gated `next_op` arms; not a labelled generate)
- Banked registers: `core/register_file_two_bank.vhd` (`BANKED`)
- SH-4 ISA overlay: `decode/gen-go/spec/sh4/{mmu,bank,exceptions}.toml`
- Configs: `core/cpu_config_{common,j1,j2,j4,sim}.vhd`, `synth/cpu_synth_j4_config.vhd`, `synth/cpu_cache_timing_config.vhd`
