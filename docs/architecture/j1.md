...
- **Fit small FPGAs.** Drive the LUT4 / cell count down far enough to leave room
  for caches, bus and peripherals on an up5k-class device.
...
Together these brought J1 within the up5k budget: **≈4945 / 5280 LC (93 %), 9 EBR,
0 DSP, routed at ~12.86 MHz.** A separate J1 *decoder subset* effort (dropping
CAS.L + 8 coprocessor ops to an illegal trap) trims further while keeping J2/J4
byte-identical.
...
- Synth config: `synth/cpu_synth_j1_config.vhd` (`cpu_synth_j1`)
- Sequential multiplier: `core/mult.vhm` (`architecture seq`)
