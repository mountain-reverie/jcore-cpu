...
architecture behaviour of cpu_dualcore_tb is
  ...
  signal d0_o, d1_o, i0_o, i1_o : cpu_data_o_t;
  signal d0_i, d1_i, i0_i, i1_i : cpu_data_i_t;

  -- cascade intermediates + final memory master
  signal ab_o, cd_o, mem_o : cpu_data_o_t;   -- ab=merge(d0,i0), cd=merge(d1,i1)
  signal ab_i, cd_i, mem_i : cpu_data_i_t;
...
begin
  ...
  u_ic0: entity work.icache_adapter port map(
    ...
    dbus_o => i0_o, dbus_ddrburst => open,
    dbus_i => i0_i, dbus_ack_r => i0_i.ack);
  u_ic1: entity work.icache_adapter port map(
    clk125 => clk, clk200 => clk, rst => rst, ctrl => cctrl,
    ibus_o => cpu1_inst_o, ibus_i => cpu1_inst_i,
    dbus_o => i1_o, dbus_ddrburst => open,
    dbus_i => i1_i, dbus_ack_r => i1_i.ack);

  -- 4:1 merge via cascaded 2-master muxes:
  --   ab=merge(d0,i0), cd=merge(d1,i1), mem=merge(ab,cd)
  m_ab: entity work.multi_master_bus_mux port map(rst => rst, clk => clk,
    m1_i => d0_i, m1_o => d0_o, m2_i => i0_i, m2_o => i0_o,
    slave_i => ab_i, slave_o => ab_o);
  m_cd: entity work.multi_master_bus_mux port map(rst => rst, clk => clk,
    m1_i => d1_i, m1_o => d1_o, m2_i => i1_i, m2_o => i1_o,
    slave_i => cd_i, slave_o => cd_o);
  m_top: entity work.multi_master_bus_mux port map(rst => rst, clk => clk,
    m1_i => ab_i, m1_o => ab_o, m2_i => cd_i, m2_o => cd_o,
    slave_i => mem_i, slave_o => mem_o);
