...
entity icache_cacheable_mux_tb is end entity;
...
begin
  clk <= not clk after 4 ns when not done else '0';
  rst <= '1', '0' after 15 ns;

  dut : entity work.icache_cacheable_mux
    port map (clk125 => clk, clk200 => clk, rst => rst, ctrl => ctrl,
              ibus_o => ibus_o, a_mmu => a_mmu, ibus_i => ibus_i,
              mem_o => mem_o, mem_ddrburst => mem_ddrburst,
              mem_i => mem_i, mem_ack_r => mem_ack_r);

  -- behavioral 1-cycle memory: ack any read, always return MEMWORD.
  process(clk) begin
    if rising_edge(clk) then
      if mem_o.en = '1' and mem_o.rd = '1' then
        mem_i.ack <= '1'; mem_i.d <= MEMWORD;
      else
        mem_i.ack <= '0'; mem_i.d <= (others => '0');
      end if;
    end if;
  end process;
  mem_ack_r <= mem_i.ack;
  ...
  stim : process
    procedure fetch(a : std_logic_vector(31 downto 1); at, c : std_logic;
                    expect : std_logic_vector(15 downto 0); nm : string) is
      variable cyc : integer := 0;
    begin
      a_mmu  <= (pa_tag => (others => '0'), at => at, c => c);
      ibus_o <= (en => '1', a => a, jp => '0');
      loop wait until rising_edge(clk); cyc := cyc + 1;
        exit when ibus_i.ack = '1'; assert cyc < 50 report nm & ": no ack" severity failure;
      end loop;
      assert ibus_i.d = expect
        report nm & ": got " & integer'image(to_integer(unsigned(ibus_i.d))) severity failure;
      ibus_o <= NULL_INST_O; a_mmu <= MMU_CACHE_I_RESET; wait until rising_edge(clk);
    end procedure;
  begin
    wait until rst = '0';
    ...
    report "icache_cacheable_mux_tb: all bypass tests passed" severity note;
    done <= true; wait;
  end process;
end architecture;
