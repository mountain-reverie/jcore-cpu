...
architecture arch of icache_modereg is
  signal this_c : cache_modereg_reg_t;
  signal this_r : cache_modereg_reg_t := CACHEMODEREG_REG_RESET;
begin
  this : process(this_r, db_i, cpu0_ddr_ibus_o, cpu1_ddr_ibus_o, ddr_status)
     variable this : cache_modereg_reg_t;
  begin
     this := this_r;
    -- reg read
...
this_c <= this;
  ...
  this_r0 : process(clk, rst)
  begin
     if rst = '1' then
        this_r <= CACHEMODEREG_REG_RESET;
...
-- db_o.d( 7 downto 2) <= (others => '0');
  ...
  -- -- ack ---------------------
  db_o.ack <= db_i.en;
  cache0_ctrl_ic.en <= this_r.ic0_en;
  cache0_ctrl_ic.inv <= this_r.ic0_inv;
  -- do the same to icache1
  -- TODO: Control independently?
  cache1_ctrl_ic.en <= this_r.ic1_en;
  cache1_ctrl_ic.inv <= this_r.ic1_inv;
  cache0_ctrl_dc.en <= this_r.dc0_en;
  cache0_ctrl_dc.inv <= this_r.dc0_inv;
  cache1_ctrl_dc.en <= this_r.dc1_en;
  cache1_ctrl_dc.inv <= this_r.dc1_inv;
  int0 <= this_r.int0;
  int1 <= this_r.int1;
  cache01sel_ctrl_temp <= this_r.cache01sel_ctrl_temp; -- temporal
end arch;
