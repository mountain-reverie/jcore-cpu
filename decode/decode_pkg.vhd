...
package decode_pack is
    ...
    constant system_instr_rom_addrs : system_instr_addr_array := (BREAK => x"fa", ERROR => x"f1", GENERAL_ILLEGAL => x"d1", INTERRUPT => x"e8", PAGE_FAULT_D => x"00", PAGE_FAULT_I => x"00", RESET_CPU => x"e1", SLOT_ILLEGAL => x"d9", TLB_DMISS_R => x"00", TLB_DMISS_W => x"00", TLB_DPROT_R => x"00", TLB_DPROT_W => x"00", TLB_IMISS => x"00", TLB_IPROT => x"00");
