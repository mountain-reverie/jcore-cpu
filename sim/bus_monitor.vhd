...
begin
  ...
  timeout_cnt_i : component timeout_cnt
    port map (
      clk     => clk,
      rst     => rst,
      enable  => cpu_bus_o.en,
  ...
  begin
    ...
    if (not fault'event and (cpu_bus_o.en = '0')) then
      if (cpu_bus_i.ack = '0') then
        report "Enable did not see ACK for " & memblock
          severity warning;
      end if;
    end if;

    if (not cpu_bus_o.en'event and (fault = '1')) then
      report "ACK timeout - do not reach in time for " & memblock
        severity warning;
    end if;

  end process;

  monitor11 : process (cpu_bus_o.a, cpu_bus_o.en) is
  begin

    if (cpu_bus_o.a'event and (cpu_bus_i.ack = '0') and (cpu_bus_o.en = '1') and not cpu_bus_o.en'event) then
      report "Address changed but did not see ACK for " & memblock
        severity warning;
    end if;

  end process;

  monitor2 : process (cpu_bus_i.ack) is
  begin

    if (cpu_bus_i.ack'event and (cpu_bus_i.ack = '1') and (cpu_bus_o.en = '0')) then
      report "ACK raises while Enable low for " & memblock
        severity warning;
    end if;
  ...
  begin
    ...
    if (not cpu_bus_o.en'event and (cpu_bus_i.ack = '0') and (cpu_bus_o.en = '0')) then
      if (cpu_bus_o.en'last_event >= 10 ns) then
        report "ACK falling delay is greater than 1 CC for " & memblock
          severity warning;
      end if;
    end if;
  ...
  monitor4 : process (cpu_bus_o.rd) is
  begin

    if (cpu_bus_o.rd'event and (cpu_bus_o.rd = '0') and (cpu_bus_i.ack = '0')) then
      report "Rd did not see ACK for " & memblock
        severity warning;
    end if;

  end process;

  monitor5 : process (cpu_bus_o.wr) is
  begin

    if (cpu_bus_o.wr'event and (cpu_bus_o.wr = '0') and (cpu_bus_i.ack = '0')) then
      report "Wr did not see ACK for " & memblock
        severity warning;
    end if;

  end process;

  monitoren : process (cpu_bus_o) is
  begin
      ...
      if (cpu_bus_o.wr = '1') then
        ...
        for i in 0 to 31 loop
          ...
          if ((cpu_bus_o.a(i) /= '0') and (cpu_bus_o.a(i) /= '1')) then
            report "Writing without address " & memblock
              severity warning;
            exit;
          end if;
        ...
        for i in 0 to 31 loop
          ...
          if ((cpu_bus_o.d(i) /= '0') and (cpu_bus_o.d(i) /= '1')) then
            report "Writing without data " & memblock
              severity warning;
            exit;
          end if;
        ...
        for i in 0 to 3 loop
          ...
          if ((cpu_bus_o.we(i) /= '0') and (cpu_bus_o.we(i) /= '1')) then
            report "Writing without Byte lane enable " & memblock
              severity warning;
            exit;
          end if;
        ...
        for i in 0 to 31 loop
          ...
          if ((cpu_bus_o.a(i) /= '0') and (cpu_bus_o.a(i) /= '1')) then
            report "Reading without address " & memblock
              severity warning;
            exit;
          end if;
        ...
        report "Enable with no Rd and no Wr " & memblock
          severity warning;
      end if;
    end if;
  ...
  begin
    ...
    for i in 0 to 31 loop
      ...
      if (cpu_bus_o.a(i) = 'X') then
        report "address has an X for " & memblock
          severity warning;
        exit;
      end if;
    ...
    if (cpu_bus_o.en = 'X') then
      report "enable is has X for " & memblock
        severity warning;
    end if;

    if (cpu_bus_o.rd = 'X') then
      report "Read is has X for " & memblock
        severity warning;
    end if;

    if (cpu_bus_o.wr = 'X') then
      report "Write has X for " & memblock
        severity warning;
    end if;
    ...
    for i in 0 to 3 loop
      ...
      if (cpu_bus_o.we(i) = 'X') then
        report "Byte lane Write Enable has an X for " & memblock
          severity warning;
        exit;
      end if;
    ...
    for i in 0 to 31 loop
      ...
      if (cpu_bus_o.d(i) = 'X') then
        report "Write data has an X for " & memblock
          severity warning;
        exit;
      end if;
    ...
    for i in 0 to 31 loop
      ...
      if (cpu_bus_i.d(i) = 'X') then
        report "Data readback has an X for " & memblock
          severity warning;
        exit;
      end if;
    ...
    if (cpu_bus_i.ack = 'X') then
      report "ACK is has X for " & memblock
        severity warning;
    end if;
