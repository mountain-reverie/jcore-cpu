...
package decode_pack is
    ...
    type mmu_reg_sel_t is (SEL_PTEH, SEL_PTEL, SEL_ASIDR, SEL_MMUCR, SEL_TTB, SEL_TEA, SEL_TSBPTR);
    ...
    type xbus_sel_t is (SEL_IMM, SEL_REG, SEL_PC, SEL_TLBPC);
    type ybus_sel_t is (SEL_IMM, SEL_REG, SEL_MACH, SEL_MACL, SEL_PC, SEL_SR, SEL_EXPEVT, SEL_INTEVT, SEL_TRA, SEL_MMU, SEL_TLBSR);
    ...
    constant DEC_ADDR_BITS : natural := 9;
    ...
    constant DEC_CORE_ROM_RESET : decode_core_reg_t := (maskint => '0', delay_slot => '0', id_stall => '0', instr_seq_zero => '0', op => (plane => SYSTEM_INSTR, code => x"0300", addr => "011100010"), ilevel => x"0");
    type system_instr_t is (BREAK, ERROR, GENERAL_ILLEGAL, INTERRUPT, PAGE_FAULT_D, PAGE_FAULT_I, RESET_CPU, SLOT_ILLEGAL, TLB_DMISS_R, TLB_DMISS_W, TLB_DPROT_R, TLB_DPROT_W, TLB_IMISS, TLB_IPROT);
    type system_instr_addr_array is array (system_instr_t range <>) of std_logic_vector(8 downto 0);
    constant system_instr_rom_addrs : system_instr_addr_array := (BREAK => "011111010", ERROR => "011110001", GENERAL_ILLEGAL => "011010001", INTERRUPT => "011101000", PAGE_FAULT_D => "100000110", PAGE_FAULT_I => "011111110", RESET_CPU => "011100001", SLOT_ILLEGAL => "011011001", TLB_DMISS_R => "000000000", TLB_DMISS_W => "000000000", TLB_DPROT_R => "000000000", TLB_DPROT_W => "000000000", TLB_IMISS => "000000000", TLB_IPROT => "000000000");
    type system_instr_code_array is array (system_instr_t range <>) of std_logic_vector(11 downto 8);
    constant system_instr_codes : system_instr_code_array := (BREAK => x"2", ERROR => x"1", GENERAL_ILLEGAL => x"7", INTERRUPT => x"0", PAGE_FAULT_D => x"5", PAGE_FAULT_I => x"4", RESET_CPU => x"3", SLOT_ILLEGAL => x"6", TLB_DMISS_R => x"9", TLB_DMISS_W => x"A", TLB_DPROT_R => x"C", TLB_DPROT_W => x"D", TLB_IMISS => x"8", TLB_IPROT => x"B");
