...
// EmitDSideDSlotImages emits the DSideDSlot axis. Unlike the I-fetch axes,
// the delay-slot memory instruction here runs straight-line from P1 (no
// translated fetch involved -- only its DATA access DMISSes), so none of the
// co-sim's cumulative-translated-fetch ceiling that forces IFetch/IFetchDSlot
// into multiple sub-images applies. The whole axis fits in ONE image; this
// still returns a slice (mirroring EmitIFetchDSlotImages's signature so
...
func EmitDSideDSlotImages(classes []Class) ([]string, error) {
	...
	for _, c := range classes {
		id := n + 1
		block, disp, err := emitCase(c, id, DSideDSlot)
		if IsSkip(err) {
			manifest.WriteString(block)
			continue
		}
	...
	var b strings.Builder
	b.WriteString("! GENERATED by faultgen (M8). Axis=DSideDSlot. Do not edit.\n")
	b.WriteString("! DSideDSlot axis: each case plants [bra L ; mem-instr] and runs straight-line\n")
	b.WriteString("! from P1 (no fetch fault); the delay-slot instruction's DATA access points at\n")
	b.WriteString("! the cold workload page and DMISSes. Restart must land on the branch (re-\n")
	b.WriteString("! issuing the delay slot); the per-form base register (@Rm+ post-inc / @-Rn\n")
	b.WriteString("! pre-dec) restore must apply exactly once. {base reg, dest/probe} snapshot\n")
	b.WriteString("! cold-vs-warm catches any double auto-modify or lost restore.\n")
	b.WriteString("! No cumulative-fetch ceiling applies (straight-line P1 execution), so this\n")
	b.WriteString("! axis is a single image, unlike IFetch/IFetchDSlot.\n")
	b.WriteString("! Manifest of excluded (skipped) instructions:\n")
	if manifest.Len() == 0 {
		b.WriteString("! (none)\n")
	} else {
