...
architecture beh of icache_ccl is
  signal this_c : icacheccl_reg_t;
  signal this_r : icacheccl_reg_t := ICACHECCLK_REG_RESET;
...
begin
  p0 : process(ra,a,mtoc,icccra,this_r)
    variable this : icacheccl_reg_t;
  -- ---------------------------------------------------------------------------
  -- variables
  -- ---------------------------------------------------------------------------
  ...
  begin -- begin of process ( )
    ...
    if MMU_ARCH and this.a_prev.mmu.at = '1' then
      tag_to_compare := this.a_prev.mmu.pa_tag;
    else
      tag_to_compare := (CACHE_PA_TAG_WIDTH-CACHE_TAG_WIDTH-1 downto 0 => '0')
                        & to_cache_tag(this.a_prev.a);
    end if;
    if(((this.aen_del1 = '1') and (ra.tag = tag_to_compare)) or
...
this_c <= this;
  ...
  ry_ccl.ten <= '1';
  ry_ccl.tag <= this_r.ma0_pa_tag when MMU_ARCH and this_r.ma0_at = '1'
